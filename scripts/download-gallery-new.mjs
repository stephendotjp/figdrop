...
for (const p of detail) {
  ...
  for (const url of p.images.slice(0, CAP)) {
    ...
    await sleep(jitter(600, 1300)); // gentle between images
  }
  manifest[p.key] = local;
  console.log(`${p.key}: ${local.length} images`);
  await sleep(jitter(1500, 3000)); // gentle between products
}

writeFileSync(join(__dirname, "gallery-manifest-new.json"), JSON.stringify(manifest, null, 2));
console.log("\nWrote scripts/gallery-manifest-new.json");
await browser.close();
