// Splice the new pre-order entries into lib/data.ts, just before the `];` that
// closes the `figures` array. (Same as splice-batch.mjs, different input.)
import { readFileSync, writeFileSync } from "node:fs";
import { fileURLToPath } from "node:url";
import { dirname, join } from "node:path";

const __dirname = dirname(fileURLToPath(import.meta.url));
const entries = readFileSync(join(__dirname, "new-figures-pre.ts.txt"), "utf8").replace(/\s+$/, "");
