// Detail scrape for the new batch. Captures everything build-data needs,
// including meta_description (for real manufacturer), quantity + availability.
...
const sel = JSON.parse(readFileSync(join(__dirname, "selected-batch.json"), "utf8"));
...
writeFileSync(join(__dirname, "detail-batch.json"), JSON.stringify(results, null, 2));
console.log(`\nWrote scripts/detail-batch.json (${results.length})`);
await browser.close();
