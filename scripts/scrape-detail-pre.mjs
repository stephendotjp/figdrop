// Detail scrape for the new pre-order run (selected-new.json → detail-pre.json).
// Same capture as scrape-detail-batch.mjs (meta_description, quantity, availability).
...
const sel = JSON.parse(readFileSync(join(__dirname, "selected-new.json"), "utf8"));
...
for (const item of sel) {
  ...
  const data = await page.evaluate(() => {
    ...
    return {
      ...
      manufacturer_tag: man?.getAttribute("alt") || man?.textContent?.trim() || "",
  ...
  console.log(`${item.key.padEnd(34)} qty=${String(data.quantity).padStart(4)} ${data.availability.padEnd(20)} ${data.images.length}img ${data.available_date || "—"}`);
  results.push({ key: item.key, url: item.url, category: item.category, ...data });
}

writeFileSync(join(__dirname, "detail-pre.json"), JSON.stringify(results, null, 2));
console.log(`\nWrote scripts/detail-pre.json (${results.length})`);
await browser.close();
